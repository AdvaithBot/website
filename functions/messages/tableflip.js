...
exports.function = async function(client, message, command) {
  ...
  let x = await message.channel.createWebhook(name, { avatar: message.author.displayAvatarURL({format: format, size: 1024}), reason: `${message.author.tag} used /tableflip on Android`})
  ...
  await x.send(`${message.content.split(' ').slice(1).join(' ')} (╯°□°）╯︵ ┻━┻`)
  await x.delete()
